...
export function Worktrees({ projectId }: WorktreesProps) {
  const { t } = useTranslation(['worktrees', 'common', 'dialogs']);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setError(result.error || t('worktrees:errors.deleteFailedGeneric'));
      }
    } catch (err) {
      setError(err instanceof Error ? err.message : t('worktrees:errors.deleteFailedGeneric'));
    } finally {
      setIsDeleting(false);
    }
  };
  ...
  const handleDeleteTerminalWorktree = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setError(result.error || t('worktrees:errors.deleteFailedTerminal'));
      }
    } catch (err) {
      setError(err instanceof Error ? err.message : t('worktrees:errors.deleteFailedTerminal'));
    } finally {
  ...
  if (!selectedProject) {
    return (
      <div className="flex h-full items-center justify-center">
        <p className="text-muted-foreground">{t('worktrees:empty.selectProject')}</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex h-full flex-col p-6">
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-foreground flex items-center gap-2">
            ...
            {t('worktrees:header.title')}
          </h2>
          <p className="text-sm text-muted-foreground mt-1">
            {t('worktrees:header.description')}
          </p>
        </div>
            ...
            {worktrees.length > 0 && (
              <div className="space-y-4">
                <h3 className="text-sm font-medium text-muted-foreground flex items-center gap-2">
                  ...
                  {t('worktrees:taskWorktrees.title')}
                </h3>
                      ...
                      <CardContent className="pt-0">
                        ...
                        <div className="flex flex-wrap gap-4 text-sm mb-4">
                          <div className="flex items-center gap-1.5 text-muted-foreground">
                            ...
                            <span>{t('worktrees:taskWorktrees.stats.filesChanged', { count: worktree.filesChanged ?? 0 })}</span>
                          </div>
                          <div className="flex items-center gap-1.5 text-muted-foreground">
                            <ChevronRight className="h-3.5 w-3.5" />
                            <span>{t('worktrees:taskWorktrees.stats.commitsAhead', { count: worktree.commitCount ?? 0 })}</span>
                          </div>
                          <div className="flex items-center gap-1.5 text-success">
                            <Plus className="h-3.5 w-3.5" />
                            <span>{t('worktrees:taskWorktrees.stats.additions', { count: worktree.additions ?? 0 })}</span>
                          </div>
                          <div className="flex items-center gap-1.5 text-destructive">
                            <Minus className="h-3.5 w-3.5" />
                            <span>{t('worktrees:taskWorktrees.stats.deletions', { count: worktree.deletions ?? 0 })}</span>
                          </div>
                        </div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          ...
                          >
                            ...
                            {t('worktrees:actions.mergeTo', { branch: worktree.baseBranch })}
                          </Button>
                          {task && (
                            <Button
                              variant="info"
            ...
            {terminalWorktrees.length > 0 && (
              <div className="space-y-4">
                <h3 className="text-sm font-medium text-muted-foreground flex items-center gap-2">
                  ...
                  {t('worktrees:terminalWorktrees.title')}
                </h3>
                    ...
                    <CardContent className="pt-0">
                      ...
                      {wt.createdAt && (
                        <div className="text-xs text-muted-foreground mb-4">
                          {t('worktrees:terminalWorktrees.created', { date: new Date(wt.createdAt).toLocaleDateString() })}
                        </div>
                      )}
        ...
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              {t('worktrees:merge.dialogTitle')}
            </DialogTitle>
            <DialogDescription>
              {t('worktrees:merge.dialogDescription')}
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="py-4">
              <div className="rounded-lg bg-muted p-4 text-sm space-y-3">
                <div className="flex items-center justify-between">
                  <span className="text-muted-foreground">{t('worktrees:merge.sourceBranch')}</span>
                ...
                <div className="flex items-center justify-between">
                  <span className="text-muted-foreground">{t('worktrees:merge.targetBranch')}</span>
                  <span className="font-mono">{selectedWorktree.baseBranch}</span>
                </div>
                <div className="border-t border-border pt-3 mt-3">
                  <div className="flex items-center justify-between text-xs">
                    <span className="text-muted-foreground">{t('worktrees:merge.changes')}</span>
                    <span>
                      {t('worktrees:merge.changesDetail', { commits: selectedWorktree.commitCount ?? 0, files: selectedWorktree.filesChanged ?? 0 })}
                    </span>
                  </div>
                </div>
              </div>
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    <p className={`font-medium ${mergeResult.success ? 'text-success' : 'text-destructive'}`}>
                      {mergeResult.success ? t('worktrees:merge.successTitle') : t('worktrees:merge.failTitle')}
                    </p>
                    <p className="text-muted-foreground mt-1">{mergeResult.message}</p>
                    {mergeResult.conflictFiles && mergeResult.conflictFiles.length > 0 && (
                      <div className="mt-2">
                        <p className="text-xs font-medium">{t('worktrees:merge.conflictingFiles')}</p>
          ...
          <DialogFooter>
            ...
            >
              {mergeResult ? t('worktrees:merge.close') : t('worktrees:merge.cancel')}
            </Button>
            {!mergeResult && (
              <Button
                onClick={handleMerge}
      ...
      <AlertDialog open={showDeleteConfirm} onOpenChange={(open) => !open && !isDeleting && setShowDeleteConfirm(false)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t('worktrees:delete.confirmTitle')}</AlertDialogTitle>
            <AlertDialogDescription>
              {t('worktrees:delete.confirmDescription')}
              {worktreeToDelete && (
                <span className="block mt-2 font-mono text-sm">
                  {worktreeToDelete.isOrphaned ? t('common:labels.orphaned') : worktreeToDelete.branch}
                </span>
              )}
              {t('worktrees:delete.cannotUndo')}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel disabled={isDeleting}>{t('worktrees:delete.cancel')}</AlertDialogCancel>
            <AlertDialogAction
              onClick={(e) => {
                e.preventDefault();
                handleDelete();
      ...
      <AlertDialog open={!!terminalWorktreeToDelete} onOpenChange={(open) => !open && !isDeletingTerminal && setTerminalWorktreeToDelete(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t('worktrees:delete.terminalTitle')}</AlertDialogTitle>
            <AlertDialogDescription>
              {t('worktrees:delete.terminalDescription')}
          ...
          <AlertDialogFooter>
            <AlertDialogCancel disabled={isDeletingTerminal}>{t('worktrees:delete.cancel')}</AlertDialogCancel>
            <AlertDialogAction
              onClick={(e) => {
                e.preventDefault();
