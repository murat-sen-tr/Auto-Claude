...
export function BatchReviewWizard({
  ...
  const renderIntro = () => (
    <div className="flex flex-col items-center justify-center py-8 space-y-6">
      ...
      <div className="text-center space-y-2">
        <h3 className="text-lg font-semibold">{t('batchWizard.intro.title')}</h3>
        <p className="text-sm text-muted-foreground max-w-md">
          {t('batchWizard.intro.description')}
        </p>
      </div>
      {analysisError && (
      ...
      <Button onClick={onStartAnalysis} size="lg">
        ...
        {t('batchWizard.intro.startButton')}
      </Button>
    </div>
  );
  ...
  const renderAnalyzing = () => (
    <div className="flex flex-col items-center justify-center py-8 space-y-6">
      ...
      <div className="text-center space-y-2">
        <h3 className="text-lg font-semibold">{t('batchWizard.analyzing.title')}</h3>
        <p className="text-sm text-muted-foreground">
          {analysisProgress?.message || t('batchWizard.analyzing.message')}
        </p>
      </div>
      <div className="w-full max-w-md">
        <Progress value={analysisProgress?.progress ?? 0} />
        <p className="text-xs text-center text-muted-foreground mt-2">
          {t('batchWizard.analyzing.complete', { progress: analysisProgress?.progress ?? 0 })}
        </p>
      </div>
    </div>
  );
      ...
      <div className="flex flex-col h-[60vh]">
        ...
        <div className="flex items-center justify-between p-3 bg-muted/50 rounded-lg mb-4">
          <div className="flex items-center gap-4 text-sm">
            <span>
              {t('batchWizard.review.stats.analyzed', { count: totalIssues })}
            </span>
            <span className="text-muted-foreground">|</span>
            <span>
              {t('batchWizard.review.stats.batches', { count: proposedBatches.length })}
            </span>
            <span className="text-muted-foreground">|</span>
            <span>
              {t('batchWizard.review.stats.single', { count: singleIssues.length })}
            </span>
          </div>
          <div className="flex items-center gap-2">
            <Button variant="ghost" size="sm" onClick={selectAllBatches}>
              {t('batchWizard.review.selectAll')}
            </Button>
            <Button variant="ghost" size="sm" onClick={deselectAllBatches}>
              {t('batchWizard.review.deselectAll')}
            </Button>
          </div>
        </div>
          ...
          {singleIssues.length > 0 && (
            <div className="mt-6">
              <h4 className="text-sm font-medium text-muted-foreground mb-2">
                {t('batchWizard.review.singleIssues')}
              </h4>
              <div className="grid grid-cols-2 gap-2">
                ...
                {singleIssues.length > 10 && (
                  <div className="p-2 text-sm text-muted-foreground">
                    {t('batchWizard.review.andMore', { count: singleIssues.length - 10 })}
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="flex items-center justify-between pt-4 mt-4 border-t border-border">
          <div className="text-sm text-muted-foreground">
            {t('batchWizard.review.selectedSummary', { batchCount: selectedCount, issueCount: totalIssuesInSelected, count: selectedCount })}
            {selectedSingleIssueNumbers.size > 0 && (
              <> {t('batchWizard.review.withSingleIssues', { count: selectedSingleIssueNumbers.size })}</>
            )}
          </div>
        </div>
      </div>
  ...
  const renderApproving = () => (
    <div className="flex flex-col items-center justify-center py-8 space-y-6">
      <Loader2 className="h-12 w-12 text-primary animate-spin" />
      <div className="text-center space-y-2">
        <h3 className="text-lg font-semibold">{t('batchWizard.approving.title')}</h3>
        <p className="text-sm text-muted-foreground">
          {t('batchWizard.approving.description')}
        </p>
      </div>
    </div>
  );
  ...
  const renderDone = () => (
    <div className="flex flex-col items-center justify-center py-8 space-y-6">
      ...
      <div className="text-center space-y-2">
        <h3 className="text-lg font-semibold">{t('batchWizard.done.title')}</h3>
        <p className="text-sm text-muted-foreground">
          {t('batchWizard.done.description')}
        </p>
      </div>
      <Button onClick={onClose}>
        {t('batchWizard.done.closeButton')}
      </Button>
    </div>
  );
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-3xl">
        <DialogHeader>
          ...
          <DialogDescription>
            {step === 'intro' && t('batchWizard.steps.intro')}
            {step === 'analyzing' && t('batchWizard.steps.analyzing')}
            {step === 'review' && t('batchWizard.steps.review')}
            {step === 'approving' && t('batchWizard.steps.approving')}
            {step === 'done' && t('batchWizard.steps.done')}
          </DialogDescription>
        </DialogHeader>
            ...
            >
              {isApproving ? (
                <>
                  ...
                  {t('batchWizard.approving.button')}
                </>
              ) : (
                <>
                  <Play className="h-4 w-4 mr-2" />
                  {t('batchWizard.footer.approve', { count: selectedBatchIds.size + selectedSingleIssueNumbers.size })}
                </>
              )}
            </Button>
          </DialogFooter>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <Badge variant="outline" className="text-xs">
                ...
                {t('batchWizard.review.issues', { count: batch.issueCount })}
              </Badge>
              <Badge
              ...
              >
                ...
                <span className={confidenceColor}>
                  {t('batchWizard.review.similar', { percent: Math.round(batch.confidence * 100) })}
                </span>
              </Badge>
            </div>
          </div>
              ...
              {batch.issues.map((issue) => (
                ...
                >
                  ...
                  <span className="text-xs text-muted-foreground">
                    {t('batchWizard.review.similar', { percent: Math.round(issue.similarityToPrimary * 100) })}
                  </span>
                </div>
              ))}
            </div>
