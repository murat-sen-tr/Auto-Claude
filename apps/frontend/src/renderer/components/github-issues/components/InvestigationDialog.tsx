      ...
      window.electronAPI.getIssueComments(projectId, selectedIssue.number)
        ...
        .catch((err: unknown) => {
          ...
          setFetchCommentsError(
            err instanceof Error ? err.message : t('investigation.failedToLoadComments')
          );
        })
        .finally(() => {
          if (isMounted) {
              ...
              <div className="space-y-2 flex-1 min-h-0 flex flex-col">
                <div className="flex items-center justify-between">
                  <h4 className="text-sm font-medium flex items-center gap-2">
                    ...
                    {t('investigation.selectComments', { selected: selectedCommentIds.length, total: comments.length })}
                  </h4>
                  <Button
                    variant="ghost"
                    size="sm"
