...
// Phase label keys - will be translated using t function
const PHASE_LABEL_KEYS: Record<TaskLogPhase, string> = {
  planning: 'tasks:logs.planning',
  coding: 'tasks:logs.coding',
  validation: 'tasks:logs.validation'
};
      ...
      <div className="p-4 space-y-2">
        {isLoadingLogs ? (
          ...
          <div className="text-center text-sm text-muted-foreground py-8">
            ...
            <p>{t('tasks:logs.noLogsYet')}</p>
            <p className="text-xs mt-1">{t('tasks:logs.logsWhenRuns')}</p>
          </div>
        )}
      </div>
    </div>
          ...
          <div className="flex items-center gap-2">
            ...
            {hasEntries && (
              <span className="text-xs text-muted-foreground">
                ({phaseLog?.entries.length} {t('tasks:logs.entries')})
              </span>
            )}
          </div>
          <div className="flex items-center gap-2">
...
function LogEntry({ entry }: LogEntryProps) {
  ...
  const getToolInfo = (toolName: string) => {
    switch (toolName) {
      case 'Read':
        return { icon: FileText, label: t('tasks:logs.reading'), color: 'text-blue-500 bg-blue-500/10' };
      case 'Glob':
        return { icon: FolderSearch, label: t('tasks:logs.searchingFiles'), color: 'text-amber-500 bg-amber-500/10' };
      case 'Grep':
        return { icon: Search, label: t('tasks:logs.searchingCode'), color: 'text-green-500 bg-green-500/10' };
      case 'Edit':
        return { icon: Pencil, label: t('tasks:logs.editing'), color: 'text-purple-500 bg-purple-500/10' };
      case 'Write':
        return { icon: FileCode, label: t('tasks:logs.writing'), color: 'text-cyan-500 bg-cyan-500/10' };
      case 'Bash':
        return { icon: Terminal, label: t('tasks:logs.running'), color: 'text-orange-500 bg-orange-500/10' };
      default:
      ...
      <div className="flex flex-col">
        <div className="flex items-center gap-2">
          <div className={cn('inline-flex items-center gap-2 rounded-md px-2 py-1 text-xs', color, 'opacity-60')}>
            ...
            <span className="text-muted-foreground">{t('tasks:logs.done')}</span>
          </div>
          {hasDetail && (
            <button
        ...
        {hasDetail && (
          ...
          >
            {isExpanded ? (
              <>
                <ChevronDown className="h-2.5 w-2.5" />
                <span>{t('tasks:logs.less')}</span>
              </>
            ) : (
              <>
                <ChevronRight className="h-2.5 w-2.5" />
                <span>{t('tasks:logs.more')}</span>
              </>
            )}
          </button>
        )}
