      ...
      <div className="p-6 space-y-6">
          ...
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between">
              <CardTitle className="text-base flex items-center gap-2">
                ...
                {t('context:memories.status.title')}
              </CardTitle>
              {memoryStatus?.available ? (
                <Badge variant="outline" className="bg-success/10 text-success border-success/30">
                  <CheckCircle className="h-3 w-3 mr-1" />
                  {t('context:memories.status.connected')}
                </Badge>
              ) : (
                <Badge variant="outline" className="bg-muted text-muted-foreground">
                  <XCircle className="h-3 w-3 mr-1" />
                  {t('context:memories.status.notAvailable')}
                </Badge>
              )}
            </div>
          </CardHeader>
            ...
            {memoryStatus?.available ? (
              <>
                <div className="grid gap-3 sm:grid-cols-2 text-sm">
                  <InfoItem label={t('context:memories.status.database')} value={memoryStatus.database || t('context:memories.status.defaultDatabase')} />
                  <InfoItem label={t('context:memories.status.path')} value={memoryStatus.dbPath || t('context:memories.status.defaultPath')} />
                </div>
                  ...
                  <div className="pt-3 border-t border-border/50">
                    <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
                      <div className="text-center p-2 rounded-lg bg-muted/30">
                        ...
                        <div className="text-xs text-muted-foreground">{t('context:memories.stats.total')}</div>
                      </div>
                      <div className="text-center p-2 rounded-lg bg-cyan-500/10">
                        ...
                        <div className="text-xs text-muted-foreground">{t('context:memories.stats.prReviews')}</div>
                      </div>
                      <div className="text-center p-2 rounded-lg bg-amber-500/10">
                        <div className="text-lg font-semibold text-amber-400">{memoryCounts.sessions}</div>
                        <div className="text-xs text-muted-foreground">{t('context:memories.stats.sessions')}</div>
                      </div>
                      <div className="text-center p-2 rounded-lg bg-blue-500/10">
                        <div className="text-lg font-semibold text-blue-400">{memoryCounts.codebase}</div>
                        <div className="text-xs text-muted-foreground">{t('context:memories.stats.codebase')}</div>
                      </div>
                      <div className="text-center p-2 rounded-lg bg-purple-500/10">
                        <div className="text-lg font-semibold text-purple-400">{memoryCounts.patterns}</div>
                        <div className="text-xs text-muted-foreground">{t('context:memories.stats.patterns')}</div>
                      </div>
                      <div className="text-center p-2 rounded-lg bg-red-500/10">
                        <div className="text-lg font-semibold text-red-400">{memoryCounts.gotchas}</div>
                        <div className="text-xs text-muted-foreground">{t('context:memories.stats.gotchas')}</div>
                      </div>
                    </div>
                  </div>
                )}
              ...
              <div className="text-sm text-muted-foreground">
                <p>{memoryStatus?.reason || t('context:memories.unavailable.reason')}</p>
                <p className="mt-2 text-xs" dangerouslySetInnerHTML={{ __html: t('context:memories.unavailable.instructions') }} />
              </div>
            )}
          </CardContent>
        </Card>
        ...
        <div className="space-y-4">
          <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider">
            {t('context:memories.search.title')}
          </h3>
          <div className="flex gap-2">
            <Input
              placeholder={t('context:memories.search.placeholder')}
          ...
          {searchResults.length > 0 && (
            <div className="space-y-3">
              <p className="text-sm text-muted-foreground">
                {t('context:memories.search.resultsFound', { count: searchResults.length })}
              </p>
                  ...
                  <CardContent className="pt-4">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-xs text-muted-foreground">
                        {t('context:memories.search.score', { score: result.score.toFixed(2) })}
                      </span>
                    </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider">
              {t('context:memories.browser.title')}
            </h3>
            <span className="text-xs text-muted-foreground">
              {t('context:memories.browser.memoryCount', { filtered: filteredMemories.length, total: recentMemories.length })}
            </span>
          </div>
          ...
          {!memoriesLoading && filteredMemories.length === 0 && recentMemories.length === 0 && (
            <div className="flex flex-col items-center justify-center py-8 text-center">
              ...
              <p className="text-sm text-muted-foreground">
                {t('context:memories.empty.noMemories')}
              </p>
            </div>
          )}
          ...
          {!memoriesLoading && filteredMemories.length === 0 && recentMemories.length > 0 && (
            <div className="flex flex-col items-center justify-center py-8 text-center">
              <Brain className="h-10 w-10 text-muted-foreground mb-3" />
              <p className="text-sm text-muted-foreground">
                {t('context:memories.empty.noFilteredMemories')}
              </p>
              <Button
                variant="link"
                size="sm"
                ...
                className="mt-2"
              >
                {t('context:memories.empty.showAll')}
              </Button>
            </div>
          )}
