    ...
    case 'approve':
      return (
        <Badge className="bg-green-500/10 text-green-400 border-green-500/30 gap-1">
          ...
          {t('context:prReview.badge.approved')}
        </Badge>
      );
    case 'request_changes':
      return (
        <Badge className="bg-amber-500/10 text-amber-400 border-amber-500/30 gap-1">
          ...
          {t('context:prReview.badge.changesRequested')}
        </Badge>
      );
    case 'comment':
      return (
        <Badge className="bg-blue-500/10 text-blue-400 border-blue-500/30 gap-1">
          ...
          {t('context:prReview.badge.commented')}
        </Badge>
      );
    default:
      return (
...
function SeverityBadge({ severity, count }: { severity: string; count: number }) {
  ...
  return (
    <Badge className={`${colorMap[severity] || 'bg-muted'} text-xs font-mono`}>
      {count} {t(`context:prReview.severity.${severity}`)}
    </Badge>
  );
}
              ...
              <div className="flex items-center gap-2 mt-2 flex-wrap">
                ...
                {totalFindings > 0 && (
                  <span className="text-xs text-muted-foreground">
                    {t('context:prReview.details.findings', { count: totalFindings })}
                  </span>
                )}
              </div>
            ...
            >
              {expanded ? (
                <>
                  ...
                  {t('context:prReview.actions.collapse')}
                </>
              ) : (
                <>
                  <ChevronDown className="h-4 w-4" />
                  {t('context:prReview.actions.details')}
                </>
              )}
            </Button>
          )}
            ...
            {hasFindings && (
              <div>
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-sm font-medium text-foreground">{t('context:prReview.details.keyFindings')}</span>
                ...
                <div className="space-y-2 pl-6">
                  ...
                  {parsed.keyFindings.length > 5 && (
                    <p className="text-xs text-muted-foreground">
                      {t('context:prReview.details.moreFindings', { count: parsed.keyFindings.length - 5 })}
                    </p>
                  )}
                </div>
              </div>
            ...
            {hasGotchas && (
              <div>
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-sm font-medium text-foreground">{t('context:prReview.details.gotchasDiscovered')}</span>
                  <Badge variant="secondary" className="text-xs px-1.5 py-0">
            ...
            {hasPatterns && (
              <div>
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-sm font-medium text-foreground">{t('context:prReview.details.patternsIdentified')}</span>
                  <Badge variant="secondary" className="text-xs px-1.5 py-0">
            ...
            {parsed.repo && parsed.prNumber && (
              <div className="pt-2">
                ...
                >
                  ...
                  {t('context:prReview.actions.viewOnGitHub')}
                </Button>
              </div>
            )}
          </div>
