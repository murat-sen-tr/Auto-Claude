        ...
        <>
          ...
          {settings.graphitiMcpEnabled && (
            <div className="space-y-2 ml-6">
              ...
              <Input
                placeholder={t('memory.mcpServerUrlPlaceholder')}
          ...
          <div className="space-y-2">
            ...
            >
              <SelectTrigger>
                <SelectValue placeholder={t('memory.embeddingProviderPlaceholder')} />
              </SelectTrigger>
              <SelectContent>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <Label className="text-sm font-medium text-foreground">
                  {envConfig.openaiKeyIsGlobal ? t('memory.openai.apiKeyOverride') : t('memory.openai.apiKey')}
                </Label>
              ...
              {envConfig.openaiKeyIsGlobal ? (
                ...
                <p className="text-xs text-muted-foreground">
                  {t('memory.openai.requiredDescription')}
                </p>
              )}
              <PasswordInput
          ...
          {embeddingProvider === 'voyage' && (
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                {t('memory.voyage.requiredDescription')}
              </p>
              <PasswordInput
          ...
          {embeddingProvider === 'google' && (
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                {t('memory.google.requiredDescription')}
              </p>
              <PasswordInput
          ...
          {embeddingProvider === 'ollama' && (
            <div className="space-y-3">
                ...
                <div className="flex items-center gap-2">
                  {ollamaStatus === 'checking' && (
                    <span className="flex items-center gap-1 text-xs text-muted-foreground">
                      ...
                      {t('memory.ollama.checking')}
                    </span>
                  )}
                  {ollamaStatus === 'connected' && (
                    <span className="flex items-center gap-1 text-xs text-success">
                      <CheckCircle2 className="h-3 w-3" />
                      {t('memory.ollama.connected')}
                    </span>
                  )}
                  {ollamaStatus === 'disconnected' && (
                    <span className="flex items-center gap-1 text-xs text-destructive">
                      <AlertCircle className="h-3 w-3" />
                      {t('memory.ollama.notRunning')}
                    </span>
                  )}
                  <Button
                    variant="ghost"
              ...
              <div className="space-y-2">
                <Label className="text-xs text-muted-foreground">{t('memory.ollama.baseUrl')}</Label>
                <Input
                  placeholder={t('memory.ollama.baseUrlPlaceholder')}
              ...
              <div className="space-y-2">
                <Label className="text-xs text-muted-foreground">{t('memory.ollama.embeddingModel')}</Label>
                {ollamaModels.length > 0 ? (
                  ...
                  >
                    <SelectTrigger>
                      <SelectValue placeholder={t('memory.ollama.selectModel')} />
                    </SelectTrigger>
                    <SelectContent>
                  ...
                  <Input
                    placeholder={t('memory.ollama.modelPlaceholder')}
                    value={envConfig.graphitiProviderConfig?.ollamaEmbeddingModel || ''}
                    onChange={(e) => onUpdateConfig({
                      graphitiProviderConfig: {
                        ...envConfig.graphitiProviderConfig,
                ...
                <p className="text-xs text-muted-foreground">
                  {t('memory.ollama.recommendedModels')}
                </p>
              </div>

              <div className="space-y-1">
                <Label className="text-xs text-muted-foreground">{t('memory.ollama.embeddingDimension')}</Label>
                <Input
                  type="number"
                  placeholder={t('memory.ollama.embeddingDimensionPlaceholder')}
                ...
                />
                <p className="text-xs text-muted-foreground">
                  {t('memory.ollama.embeddingDimensionDescription')}
                </p>
              </div>
            </div>
          )}
